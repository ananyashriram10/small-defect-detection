"""Train ANZD-PIDNet v4: fast shape-aware small-defect segmentation.

v4 extends the working ANZD-PIDNet V1 protocol. During training only, small
...
Modes (set METHOD):
    baseline       v4 architecture control without ANZD auxiliary terms
    component      v4 + component-balanced recall loss
    zoom           v4 + area-normalized crop learning/distillation
    zoom_component full v4 training method (default)

RunPod example:
    export WANDB_API_KEY=<key>
    export DATASET_ROOT=/workspace/dataset
    export METHOD=zoom_component
    nohup python -u novelty_experiments/anzd_pidnet_v4/train_runpod.py > anzd_pidnet_v4.log 2>&1 &
    tail -f anzd_pidnet_v4.log
"""
...
MODEL_LABEL = f"ANZD-PIDNet v4 ({METHOD})"
...
MODEL_WIDTH = env_int("MODEL_WIDTH", 40)
DISTANCE_LOSS_WEIGHT = env_float("DISTANCE_LOSS_WEIGHT", 0.25)
RUN_NAME = os.environ.get("RUN_NAME", f"ANZD_PIDNet_v4_{METHOD}_imgsz{IMG_SIZE}")
...
BASE_DIR = Path(os.environ.get("BASE_DIR", "/workspace/anzd_pidnet_v4_runs"))
...
from anzd_pidnet_v4.pidnet import (  # noqa: E402
    BoundaryLoss,
    OhemCrossEntropy,
    build_pidnet_v4,
    pidnet_loss,
...
from anzd_pidnet_v4.zoom_utils import (  # noqa: E402
    SegmentationMetrics,
...
model = build_pidnet_v4(num_classes=2, augment=True, planes=MODEL_WIDTH, shape_refine=True).to(DEVICE)
trainable_parameters = sum(parameter.numel() for parameter in model.parameters() if parameter.requires_grad)
total_parameters = sum(parameter.numel() for parameter in model.parameters())
print(f"ANZD-PIDNet v4 parameters: total={total_parameters:,}, trainable={trainable_parameters:,}")
wandb.config.update(
    {
...
summary = {
    ...
    "Notes": (
        "Segmentation-only ANZD-PIDNet v4; bounded suppressive contrast gate, native 1/4-resolution "
        "context-verified foreground correction, boundary/signed-distance shape head, and lightweight "
        "1/2-resolution edge refiner are deployed in one forward pass. Area-normalized zoom branch, "
        "component loss, and threshold calibration remain training/evaluation-only. "
