"""ANZD-PIDNet v4: quality-first, single-pass small-defect segmentation.

V4 keeps PIDNet's efficient P/I/D streams but makes the refinement path more
conservative and adds a lightweight half-resolution contour refiner. The model
still performs one forward pass at inference and uses no SAM/SAM2 weights,
prompts, or external teacher. The optional zoom consistency term is ANZD's
training-only self-distillation between two views of the same model.
"""
...
class SuppressiveContrastGate(nn.Module):
    """Use local contrast as bounded, signed evidence.

    V3 added a non-negative texture response to the feature map. On the visual
    examples this made repetitive surface texture look like foreground. V4
    centers the response with ``tanh`` and learns a suppression gate: a high
    gate attenuates texture evidence, while a low gate permits a useful edge
    residual through. The branch remains shallow and 1/4-resolution.
    """
    ...
    def __init__(self, feature_channels):
        ...
        self.texture = nn.Sequential(
            ...
            # Keep the response signed. A ReLU would make every texture
            # response an additive foreground-biased signal.
            nn.Conv2d(
                feature_channels,
                feature_channels,
                3,
    ...
    def forward(self, image, quarter_features):
        ...
        suppression = self.gate(torch.cat([quarter_features, texture], dim=1))
        signed_texture = torch.tanh(texture)
        # A bounded residual keeps the new branch from overwhelming PIDNet's
        # stable semantic stream during early training.
        return quarter_features + 0.25 * (1.0 - suppression) * signed_texture


# Backwards-compatible import name for small external scripts.
LocalContrastGate = SuppressiveContrastGate
...
class ShapeRefinementHead(nn.Module):
    ...
    def __init__(self, detail_channels, num_classes, refine_channels):
        ...
        if num_classes != 2:
            raise ValueError("ANZD-PIDNet v4 currently supports binary segmentation (num_classes=2).")
...
class HalfResolutionEdgeRefiner(nn.Module):
    """Cheap 1/2-resolution contour refinement for tighter masks.

    The refiner sees the RGB image at 1/2 resolution and the semantic logits.
    It predicts a bounded one-channel foreground correction using depthwise
    spatial filtering plus pointwise mixing. It adds no second backbone and is
    used once per image.
    """
    ...
    def __init__(self, channels=24):
        ...
        self.logit_projection = nn.Sequential(
            nn.Conv2d(2, channels, 1, bias=False),
            BatchNorm2d(channels, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
        )
        self.fuse = nn.Sequential(
            nn.Conv2d(channels * 2, channels * 2, 3, padding=1, groups=channels * 2, bias=False),
            BatchNorm2d(channels * 2, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
        ...
        self.correction = nn.Conv2d(channels, 1, 1, bias=True)

    def forward(self, image, semantic_logits):
        half_size = (max(image.shape[-2] // 2, 1), max(image.shape[-1] // 2, 1))
        image_features = self.image_projection(image)
        if image_features.shape[-2:] != half_size:
            image_features = F.interpolate(
                image_features, half_size, mode="bilinear", align_corners=ALIGN_CORNERS
            )
        logits_half = F.interpolate(
            semantic_logits, half_size, mode="bilinear", align_corners=ALIGN_CORNERS
        )
        logits_features = self.logit_projection(logits_half)
        fused = self.fuse(torch.cat([image_features, logits_features], dim=1))
        coarse_foreground = torch.softmax(logits_half.float(), dim=1)[:, 1:2].to(fused.dtype)
        edge_gate = self.edge_gate(torch.cat([fused, coarse_foreground], dim=1))
        delta = 0.25 * torch.tanh(self.correction(fused)) * edge_gate
        return F.interpolate(
            torch.cat([torch.zeros_like(delta), delta], dim=1),
            image.shape[-2:],
            mode="bilinear",
...
class PIDNet(nn.Module):
    """ANZD-PIDNet v4: quality-first PIDNet with contour refinement."""
    ...
    def __init__(
        ...
        self.local_contrast = SuppressiveContrastGate(planes)
        self.relu = nn.ReLU(inplace=True)
        ...
        if augment:
            ...
            # in v4. Keep the old head only for the explicit shape_refine=False
        ...
        if edge_refine:
            self.edge_refiner = HalfResolutionEdgeRefiner(max(16, planes // 2))
        self._init_weights()
        # Start the suppressive texture path conservatively; the gate can open
        # when local contrast is useful but does not dominate epoch 1.
        nn.init.constant_(self.local_contrast.gate[-2].bias, 1.5)
        if shape_refine:
        ...
        if edge_refine:
            # Start from the stable PIDNet semantic solution and learn only a
            # bounded contour residual.
            nn.init.zeros_(self.edge_refiner.correction.weight)
            nn.init.zeros_(self.edge_refiner.correction.bias)
    ...
    def forward(self, x):
        ...
        if self.edge_refine:
            # The edge refiner returns a full-resolution signed foreground
            # correction. Keeping this last step outside the P/I/D streams
            # preserves PIDNet's efficient context computation.
            final = F.interpolate(final, size=image.shape[-2:], mode="bilinear", align_corners=ALIGN_CORNERS)
            final = final + self.edge_refiner(image, final)
        if self.augment:
...
def resize_pidnet_outputs(outputs, size):
    """Upsample all PIDNet v4 outputs to a common label resolution."""
    return [
...
def pidnet_loss(
    ...
    """PIDNet loss plus the v4 signed-distance shape objective.
    ...
    if len(outputs) >= 4:
        if distance_target is None:
            raise ValueError("distance_target is required for the four-output PIDNet v4")
...
def build_pidnet_v4(num_classes=2, augment=True, planes=40, shape_refine=True, edge_refine=True):
    return PIDNet(
        m=2,
        n=3,
...
def build_pidnet_v3(num_classes=2, augment=True, planes=40, shape_refine=True):
    """Compatibility constructor for the v3 architecture without edge refinement."""
    return build_pidnet_v4(
        num_classes=num_classes,
        augment=augment,
        planes=planes,
        shape_refine=shape_refine,
...
def build_pidnet_s(num_classes=2, augment=True):
    """Compatibility alias for the v4 default model."""
    return build_pidnet_v4(num_classes=num_classes, augment=augment)
