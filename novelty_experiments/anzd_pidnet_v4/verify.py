"""Fast ANZD-PIDNet v4 architecture, geometry, gradient, and metric verification."""
...
try:
    ...
    raise SystemExit(
        f"Missing dependency: {exc.name}. Install v4 runtime dependencies first with "
        "python -m pip install -r novelty_experiments/anzd_pidnet_v4/requirements.txt "
...
def verify_model_and_losses():
    torch.manual_seed(7)
    model = build_pidnet_v4()
    model.train()
    parameter_count = sum(parameter.numel() for parameter in model.parameters())
    assert 8_000_000 < parameter_count < 20_000_000, parameter_count
    ...
    distance = torch.stack(
        [torch.from_numpy(generate_signed_distance(label.numpy().astype(np.uint8))) for label in labels]
    )
