...
def verify_model_and_losses():
    ...
    assert 7_500_000 < parameter_count < 8_100_000, parameter_count
    ...
    foreground_dice = soft_foreground_dice_loss(outputs[1], labels)
    boundary_dice = soft_boundary_dice_loss(outputs[-1], boundary)
    lovasz = lovasz_hinge_loss(outputs[1], labels)
    hard_negative = hard_negative_suppression_loss(outputs[1], labels)
    ...
    total = base_loss + foreground_dice + boundary_dice + lovasz + hard_negative + component_loss + distillation_loss
    ...
    print(
        "model/loss: OK",
        {
            ...
            "metric_losses": [float(value.detach()) for value in (foreground_dice, boundary_dice, lovasz, hard_negative)],
            "teacher_stats": stats,
        },
    )
