"""PIDNet-S plus the lightweight V2 refinement and metric losses.

The PIDNet-S trunk and original three-part loss are ported from the project's
existing ``baselines/segmentation/PIDNet/pidnet_s_kaggle.ipynb`` baseline.
V2 adds a shallow 1/4-resolution refinement head and losses that directly
target mask overlap and high-confidence background errors.
"""
...
class PIDNet(nn.Module):
    """PIDNet-S (`m=2`, `n=3`) for two-class segmentation.

    ``refine=True`` adds a shallow high-resolution head. It remains part of
    the same single forward pass and reuses the early 1/4-resolution detail
    feature; it is not a second model or a test-time crop pass.
    """
    ...
    def __init__(
        ...
        if refine:
            # The stem output is 1/4 resolution. A depthwise-separable detail
            # block keeps this refinement inexpensive while recovering contour
            # information discarded by the 1/8-resolution PIDNet head.
            refine_channels = max(planes // 2, 8)
            self.refine_detail = nn.Sequential(
                nn.Conv2d(planes, planes, 3, padding=1, groups=planes, bias=False),
                BatchNorm2d(planes, momentum=BN_MOMENTUM),
                nn.ReLU(inplace=True),
                nn.Conv2d(planes, refine_channels, 1, bias=False),
                BatchNorm2d(refine_channels, momentum=BN_MOMENTUM),
                nn.ReLU(inplace=True),
            )
            self.refine_fuse = nn.Sequential(
                nn.Conv2d(refine_channels + num_classes, refine_channels, 3, padding=1, bias=False),
                BatchNorm2d(refine_channels, momentum=BN_MOMENTUM),
                nn.ReLU(inplace=True),
                nn.Conv2d(refine_channels, num_classes, 1, bias=True),
            )
        self._init_weights()
        if refine:
            # Start as the original PIDNet prediction and let the new head
            # learn only a residual correction during fine-tuning.
            nn.init.zeros_(self.refine_fuse[-1].weight)
            nn.init.zeros_(self.refine_fuse[-1].bias)
    ...
    def forward(self, x):
        ...
        if self.refine:
            ...
            coarse_quarter = F.interpolate(
                final, size=quarter_size, mode="bilinear", align_corners=ALIGN_CORNERS
            )
            detail = self.refine_detail(quarter_features)
            correction = self.refine_fuse(torch.cat([coarse_quarter, detail], dim=1))
            final = coarse_quarter + correction
        if self.augment:
...
def build_pidnet_s(num_classes=2, augment=True, refine=True):
    return PIDNet(
        m=2,
        n=3,
...
def soft_foreground_dice_loss(logits: torch.Tensor, target: torch.Tensor, smooth: float = 1.0):
    """Differentiable foreground Dice loss for the final semantic logits."""
    probabilities = F.softmax(logits.float(), dim=1)[:, 1]
    target_float = (target != 255).float() * target.float()
...
def soft_boundary_dice_loss(boundary_logits: torch.Tensor, target: torch.Tensor, smooth: float = 1.0):
    """Differentiable Dice loss for PIDNet's boundary branch."""
    probabilities = torch.sigmoid(boundary_logits.float()[:, 0])
...
def lovasz_hinge_loss(logits: torch.Tensor, target: torch.Tensor):
    """Binary Lovasz hinge loss, a direct surrogate for foreground IoU."""
    margins = logits.float()[:, 1] - logits.float()[:, 0]
    valid = target != 255
    labels = target.float()
    margins = margins[valid].reshape(-1)
    labels = labels[valid].reshape(-1)
    if labels.numel() == 0:
        return logits.sum() * 0.0
    signs = 2.0 * labels - 1.0
    errors = 1.0 - margins * signs
    errors_sorted, order = torch.sort(errors, descending=True)
    labels_sorted = labels[order]
    total_positive = labels_sorted.sum()
    intersection = total_positive - labels_sorted.float().cumsum(0)
    union = total_positive + (1.0 - labels_sorted).float().cumsum(0)
    jaccard = 1.0 - intersection / union.clamp_min(1.0)
    if jaccard.numel() > 1:
        jaccard[1:] = jaccard[1:] - jaccard[:-1].clone()
    return torch.dot(F.relu(errors_sorted), jaccard.detach())
