"""Train ANZD-PIDNet v3: fast shape-aware small-defect segmentation.

V3 extends the working ANZD-PIDNet V1 protocol. During training only, small
...
Modes (set METHOD):
    baseline       v3 architecture control without ANZD auxiliary terms
    component      v3 + component-balanced recall loss
    zoom           v3 + area-normalized crop learning/distillation
    zoom_component full v3 training method (default)

RunPod example:
    export WANDB_API_KEY=<key>
    export DATASET_ROOT=/workspace/dataset
    export METHOD=zoom_component
    nohup python -u novelty_experiments/anzd_pidnet_v3/train_runpod.py > anzd_pidnet_v3.log 2>&1 &
    tail -f anzd_pidnet_v3.log
"""
...
MODEL_LABEL = f"ANZD-PIDNet v3 ({METHOD})"
...
RUN_NAME = os.environ.get("RUN_NAME", f"ANZD_PIDNet_v3_{METHOD}_imgsz{IMG_SIZE}")
...
BASE_DIR = Path(os.environ.get("BASE_DIR", "/workspace/anzd_pidnet_v3_runs"))
...
from anzd_pidnet_v3.pidnet import (  # noqa: E402
    BoundaryLoss,
    OhemCrossEntropy,
    build_pidnet_v3,
    pidnet_loss,
...
from anzd_pidnet_v3.zoom_utils import (  # noqa: E402
    SegmentationMetrics,
...
model = build_pidnet_v3(num_classes=2, augment=True, planes=MODEL_WIDTH, shape_refine=True).to(DEVICE)
trainable_parameters = sum(parameter.numel() for parameter in model.parameters() if parameter.requires_grad)
total_parameters = sum(parameter.numel() for parameter in model.parameters())
print(f"ANZD-PIDNet v3 parameters: total={total_parameters:,}, trainable={trainable_parameters:,}")
wandb.config.update(
    {
...
summary = {
    ...
    "Notes": (
        "Segmentation-only ANZD-PIDNet v3; gated local-contrast stream, native 1/4-resolution "
        "detail fusion, and boundary/signed-distance shape head are deployed in one forward pass. "
        "Area-normalized zoom branch and component loss remain training-only. "
