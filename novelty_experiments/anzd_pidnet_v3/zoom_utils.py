...
def generate_signed_distance(binary_mask: np.ndarray, clip_distance: float = 32.0) -> np.ndarray:
    """Return a normalized signed distance target (+inside, -outside).

    OpenCV's distance transform is used so the target is inexpensive to build
    in the dataloader and requires no additional scientific-python dependency.
    Distances are clipped and scaled to ``[-1, 1]`` for a stable SmoothL1 loss.
    """
    mask = np.ascontiguousarray(binary_mask.astype(np.uint8))
    foreground = (mask > 0).astype(np.uint8)
    background = (foreground == 0).astype(np.uint8)
    inside = cv2.distanceTransform(foreground, cv2.DIST_L2, 3)
    outside = cv2.distanceTransform(background, cv2.DIST_L2, 3)
    signed = np.clip(inside - outside, -clip_distance, clip_distance)
    return (signed / max(float(clip_distance), 1.0)).astype(np.float32)
